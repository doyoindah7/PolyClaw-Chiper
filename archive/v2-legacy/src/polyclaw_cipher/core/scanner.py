...
CRYPTO_PATTERNS = [
    ...
    # "Bitcoin Up or Down" with window
    re.compile(
        r"(?P<asset>BTC|Bitcoin|ETH|Ethereum|SOL|Solana|BNB|XRP|Ripple|DOGE|Dogecoin)\s+"
    ...
    # "Will the price of Bitcoin be above $X on DATE?" — threshold market
    re.compile(
        r"(?:price\s+of\s+)?(?P<asset>BTC|Bitcoin|ETH|Ethereum|SOL|Solana|BNB|XRP|Ripple|DOGE|Dogecoin)\s+"
        r"(?:be\s+)?above\s+\$[\d,]+",
        re.IGNORECASE,
    ),
    # "Will Bitcoin dip to $X in MONTH?" — dip market
    re.compile(
        r"(?P<asset>BTC|Bitcoin|ETH|Ethereum|SOL|Solana|BNB|XRP|Ripple|DOGE|Dogecoin)\s+"
        ...
        for pat in CRYPTO_PATTERNS:
            ...
            if m:
                ...
                try:
                    ...
                    pass
                # Try date group → daily
                try:
                    if m.group("date"):
                        return asset, 1440
                except (IndexError, AttributeError):
                    pass
                # Pattern matched but no window/date (threshold/dip markets)
                return asset, 1440
        return None, None
